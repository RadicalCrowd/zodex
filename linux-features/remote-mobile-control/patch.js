...
function applyLinuxRemoteControlDeviceKeyPatch(source) {
  ...
  const guard = DEVICE_KEY_GUARDS.find((candidate) => source.includes(candidate)) ?? null;
  if (insertionNeedle == null || guard == null) {
  ...
  return source
    ...
    .replace(guard, `if(process.platform===\`linux\`)return codexLinuxRemoteControlDeviceKeyClient();${guard}`);
}
...
function buildLateUnknownConversationHydrationReplacement(
  ...
  const owner = target.owner ?? "this";
  const conversations = target.conversations ?? `${owner}.conversations`;
  const upsertConversationFromThread = target.upsertConversationFromThread ??
    `${owner}.upsertConversationFromThread`;
  ...
  return (
    `if(!${conversations}.get(${conversationIdVar})){/*${REMOTE_MOBILE_LATE_EVENT_HYDRATION_MARKER}*/${unknownConversationPrelude}${unknownConversationPrelude.length > 0 ? ";" : ""}` +
    `let ${pendingMapVar}=${owner}.codexLinuxRemoteMobilePendingNotifications??=new Map,${queueVar}=${pendingMapVar}.get(${conversationIdVar});` +
    `${queueVar}||(${queueVar}=[],${pendingMapVar}.set(${conversationIdVar},${queueVar})),${queueVar}.push(n);` +
    `let ${inFlightVar}=${owner}.codexLinuxRemoteMobileInFlightHydrations??=new Set;` +
    ...
    `let ${readVar}=(s=0)=>${owner}.readThread(${conversationIdVar},{includeTurns:!0}).then(e=>{let t=e?.thread??e,c=${owner}.codexLinuxRemoteMobilePendingNotifications?.get(${conversationIdVar})??[],codexLinuxRemoteMobileTurns=Array.isArray(e?.turns)?e.turns:Array.isArray(t?.turns)?t.turns:null;` +
    ...
    `${owner}.codexLinuxRemoteMobilePendingNotifications?.delete(${conversationIdVar}),${owner}.codexLinuxRemoteMobileInFlightHydrations?.delete(${conversationIdVar}),${loggerVar}.warning(\`Skipping hydration for missing conversation\`,{safe:{queuedNotificationCount:c.length},sensitive:{conversationId:${conversationIdVar}}});return}` +
    `${upsertConversationFromThread}(t),${owner}.codexLinuxRemoteMobilePendingNotifications?.delete(${conversationIdVar}),${owner}.codexLinuxRemoteMobileInFlightHydrations?.delete(${conversationIdVar});for(let codexLinuxRemoteMobileQueuedNotification of c)${owner}.onNotification(codexLinuxRemoteMobileQueuedNotification.method,codexLinuxRemoteMobileQueuedNotification.params)})` +
    ...
    `${owner}.codexLinuxRemoteMobilePendingNotifications?.delete(${conversationIdVar}),${owner}.codexLinuxRemoteMobileInFlightHydrations?.delete(${conversationIdVar}),${loggerVar}.error(\`Failed to hydrate conversation for ${eventName}\`,{safe:{},sensitive:{conversationId:${conversationIdVar},error:e}})});` +
    `${inFlightVar}.add(${conversationIdVar}),${readVar}();break}`
  );
}

function buildCurrentUnknownTurnHydrationReplacement(
  prefix,
  notificationVar,
  conversationIdVar,
  normalizerFn,
  ...
  return `${prefix}if(!${stateVar}.threadStore.conversations.get(${conversationIdVar})){/*${REMOTE_MOBILE_UNKNOWN_TURN_MARKER}*//*${REMOTE_MOBILE_NOTIFICATION_QUEUE_MARKER}*//*${REMOTE_MOBILE_IN_FLIGHT_HYDRATION_MARKER}*/let l=${notificationVar}.params?.turn?.threadId??${notificationVar}.params?.thread?.id,d=l!=null?${normalizerFn}(l):null,u=${notificationVar}.params?.turn?.id??${notificationVar}.params?.turnId;if(d==null||u!=null&&d===${normalizerFn}(u)){${loggerVar}.warning(\`Skipping hydration for ambiguous turn/started\`,{safe:{},sensitive:{conversationId:${conversationIdVar},resolvedConversationId:d,turnId:u??null}});break}${notificationVar}={...${notificationVar},params:{...${notificationVar}.params,threadId:l}};if(${stateVar}.threadStore.conversations.get(d)){${managerVar}.onNotification(${notificationVar}.method,${notificationVar}.params);break}let i=${managerVar}.codexLinuxRemoteMobilePendingNotifications??=new Map,a=i.get(d);a||(a=[],i.set(d,a));let p=u!=null?a.findIndex(e=>{let t=e.params?.turn?.id??e.params?.turnId;return e.method===${notificationVar}.method&&t!=null&&${normalizerFn}(t)===${normalizerFn}(u)}):-1;p>=0?a[p]=${notificationVar}:a.push(${notificationVar});let h=${managerVar}.codexLinuxRemoteMobileInFlightHydrations??=new Set;if(h.has(d)){${loggerVar}.warning(\`Queueing turn/started for hydrating conversation\`,{safe:{queuedNotificationCount:a.length,dedupedNotification:p>=0},sensitive:{conversationId:d}});break}${loggerVar}.warning(\`Hydrating conversation for turn/started\`,{safe:{queuedNotificationCount:a.length},sensitive:{conversationId:d}});let o=(s=0)=>${managerVar}.readThread(d,{includeTurns:!0}).then(e=>{let t=e?.thread??e,c=${managerVar}.codexLinuxRemoteMobilePendingNotifications?.get(d)??[],codexLinuxRemoteMobileTurns=Array.isArray(e?.turns)?e.turns:Array.isArray(t?.turns)?t.turns:null;if(!t||!Array.isArray(codexLinuxRemoteMobileTurns)||codexLinuxRemoteMobileTurns.length===0){if(s<12){${loggerVar}.warning(\`Retrying hydration for missing conversation\`,{safe:{queuedNotificationCount:c.length,attempt:s+1},sensitive:{conversationId:d}}),setTimeout(()=>o(s+1),250);return}${managerVar}.codexLinuxRemoteMobilePendingNotifications?.delete(d),${managerVar}.codexLinuxRemoteMobileInFlightHydrations?.delete(d),${loggerVar}.warning(\`Skipping hydration for missing conversation\`,{safe:{queuedNotificationCount:c.length},sensitive:{conversationId:d}});return}${stateVar}.upsertConversationFromThread(t),${managerVar}.codexLinuxRemoteMobilePendingNotifications?.delete(d),${managerVar}.codexLinuxRemoteMobileInFlightHydrations?.delete(d);for(let codexLinuxRemoteMobileQueuedNotification of c)${managerVar}.onNotification(codexLinuxRemoteMobileQueuedNotification.method,codexLinuxRemoteMobileQueuedNotification.params)}).catch(e=>{if(s<12){${loggerVar}.warning(\`Retrying hydration for turn/started\`,{safe:{attempt:s+1},sensitive:{conversationId:d,error:e}}),setTimeout(()=>o(s+1),250);return}${managerVar}.codexLinuxRemoteMobilePendingNotifications?.delete(d),${managerVar}.codexLinuxRemoteMobileInFlightHydrations?.delete(d),${loggerVar}.error(\`Failed to hydrate conversation for turn/started\`,{safe:{},sensitive:{conversationId:d,error:e}})});h.add(d),o();break}`;
}
...
function applyLinuxRemoteMobileConversationHydrationPatch(source) {
  ...
  if (!patched.includes(REMOTE_MOBILE_NOTIFICATION_QUEUE_MARKER)) {
    ...
    } else if (currentUnknownTurnNeedle.test(patched)) {
      patched = patched.replace(
        currentUnknownTurnNeedle,
        (_needle, prefix, _threadIdParamVar, notificationVar, conversationIdVar, normalizerFn, stateVar, managerVar) =>
          buildCurrentUnknownTurnHydrationReplacement(
            prefix,
            notificationVar,
            conversationIdVar,
            normalizerFn,
    ...
    const currentItemStartedNeedle =
      /if\(!([A-Za-z_$][\w$]*)\.threadStore\.conversations\.get\(([A-Za-z_$][\w$]*)\)\)\{([A-Za-z_$][\w$]*)\.logger\.error\(`Received item\/started for unknown conversation`,\{safe:\{conversationId:\2\},sensitive:\{\}\}\);break\}/u;
    ...
    } else if (currentItemStartedNeedle.test(patched)) {
      patched = patched.replace(
        currentItemStartedNeedle,
        (_needle, stateVar, conversationIdVar, managerVar) =>
          buildLateUnknownConversationHydrationReplacement(
            ...
            {
              owner: managerVar,
              conversations: `${stateVar}.threadStore.conversations`,
              upsertConversationFromThread: `${stateVar}.upsertConversationFromThread`,
            },
          ),
      );
    ...
    const currentItemCompletedNeedle =
      /if\(([^{};]*clearItemTerminalInputBuffer\([^{};]*\)),!([A-Za-z_$][\w$]*)\.threadStore\.conversations\.get\(([A-Za-z_$][\w$]*)\)\)\{([A-Za-z_$][\w$]*)\.logger\.error\(`Received item\/completed for unknown conversation`,\{safe:\{conversationId:\3\},sensitive:\{\}\}\);break\}/u;
    ...
    } else if (currentItemCompletedNeedle.test(patched)) {
      patched = patched.replace(
        currentItemCompletedNeedle,
        (_needle, completionPrelude, stateVar, conversationIdVar, managerVar) =>
          `${completionPrelude};${buildLateUnknownConversationHydrationReplacement(
            "item/completed",
            conversationIdVar,
            `${managerVar}.logger`,
            "",
    ...
    } else if (currentTurnCompletedNeedle.test(patched)) {
      patched = patched.replace(
        currentTurnCompletedNeedle,
        (_needle, stateVar, conversationIdVar, completionPrelude, managerVar) =>
          buildLateUnknownConversationHydrationReplacement(
            "turn/completed",
            conversationIdVar,
            `${managerVar}.logger`,
  ...
  if (currentCompletedItemDropPattern.test(source)) {
    return source.replace(
      ...
      (
        ...
        `${workItemPredicate}(${completedItemVar})&&(${turnVar}.firstTurnWorkItemStartedAtMs=${turnVar}.firstTurnWorkItemStartedAtMs??Date.now());let codexLinuxCompletedItemExists=${turnVar}.items.some(e=>e.id===${viewItemVar}.id);if(${completedItemVar}.type!==\`subAgentActivity\`&&(${completedItemVar}.type!==\`sleep\`||${conversationVar}.mode!==\`durable\`)&&codexLinuxCompletedItemExists&&!${findItemFn}(${turnVar},${completedItemVar}.id,${completedItemVar}.type,${managerVar}.logger))return;${upsertItemFn}(${turnVar},${viewItemVar})`,
    );
  }
