...
function sharedTransportClassSource(symbols) {
  return (
    ...
    "authorityArgs(){let e=[`app-server`,`--listen`,`unix://${this.socketPath}`],t=process.env.CODEX_LINUX_APP_DIR;if(typeof t!==`string`||!t.startsWith(`/`))return e;let n=require(`node:fs`),r=require(`node:path`).join(t,`.codex-linux`,`desktop-app-server-remote-control-enabled`);try{let t=n.lstatSync(r);return t.isFile()&&!t.isSymbolicLink()&&(typeof process.getuid!=`function`||t.uid===process.getuid())&&n.readFileSync(r,`utf8`)===`version=1\\nowner=desktop\\n`?[`app-server`,`--remote-control`,`--listen`,`unix://${this.socketPath}`]:e}catch(t){return e}}" +
