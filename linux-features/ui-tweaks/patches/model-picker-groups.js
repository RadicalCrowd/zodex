...
function modelPickerGroupsConfig(context) {
  const defaults = context?.feature?.manifest?.tweaks?.modelPicker?.groupedModels;
...
function classifyModelGroup(option, customCategories = null) {
  ...
  // Preserve provider categories even when the public slug is broker-prefixed.
  if (id.startsWith("kilo/") || raw.includes("/kilo/") || raw.includes("kilo-free") || raw.includes("[kilofree]")) {
    return "Kilo Free";
  }
  if (id.startsWith("opencode/") || raw.includes("/opencode/") || raw.includes("opencode-free") || raw.includes("[opencodefree]")) {
...
function modelGroupsRuntimeSource() {
  return [
    ...
    `}`,
    `}`,
    `if(id.startsWith(\`kilo/\`)||raw.includes(\`/kilo/\`)||raw.includes(\`kilo-free\`)||raw.includes(\`[kilofree]\`))return\`Kilo Free\`;`,
    `if(id.startsWith(\`opencode/\`)||raw.includes(\`/opencode/\`)||raw.includes(\`opencode-free\`)||raw.includes(\`[opencodefree]\`))return\`OpenCode Free\`;`,
